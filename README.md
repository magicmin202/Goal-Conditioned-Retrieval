...
| 로그 | specificity | actionability | goal_progress | quality_total | 비고 |
|---|---|---|---|---|---|
| 방콕 항공권 예약 완료 (28만원) | 0.70 | 0.85 | 1.00 | **0.87** | cat=booking(core) |
| 호텔 예약 확정 | 0.20 | 0.80 | 1.00 | **0.67** | cat=booking(core) |
| 여행 준비물 쇼핑 | 0.10 | 0.60 | 0.60 | **0.46** | cat=logistics(supporting) |
| 짐 준비 | 0.00 | 0.20 | 0.60 | **0.31** | cat=logistics(supporting) |

> **Note**: logistics는 travel_planning의 supporting category이므로 category gate를 통과하되,
> booking/budgeting 대비 domain_consistency가 낮아 최종 score는 자연스럽게 낮아집니다.
> 단, `evidence_strength=high` + `activity_type=execution` 로그는 actionability가 높을 수 있습니다.
