...
- priority_terms는 evidence_terms 중 이 목표를 가장 강하게 식별하는 최상위 5개 단어입니다
...
{{
  ...
  "priority_terms": ["evidence_terms 중 목표를 가장 강하게 식별하는 최상위 5개 단어"],
...
def _heuristic_expansion(goal: ResearchGoal, max_terms: int) -> dict:
    ...
    for kw, data in _HEURISTIC.items():
        if kw in key:
            evidence = data["evidence_terms"][:max_terms]
            return {
...
def _call_gemini(goal: ResearchGoal, max_terms: int, gemini_config=None) -> dict:
    ...
    parsed.setdefault("priority_terms", parsed.get("evidence_terms", [])[:5])
